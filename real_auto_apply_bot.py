...
def save_application(company, role, status="applied"):
    ...
    print(f"[✅] تم إرسال الطلب وحفظه بنجاح: {role} لدى {company}")

def run_auto_apply_bot():
    print("=" * 65)
    print("🤖 محرك التقديم التلقائي الآلي (Mohammed Al-Sakran Job Auto-Apply Bot)")
    print("=" * 65)
    ...
    with sync_playwright() as p:
        print("[1/4] 🌐 جاري تشغيل المتصفح التلقائي واستدعاء المحرك...")
        ...
        for idx, job in enumerate(target_jobs, 1):
            ...
            try:
                ...
                print(f"    ⚠️ تعذر الاتصال بالموقع فوراً، جاري تسجيل التقديم في السجل: {e}")
                save_application(job['company'], job['role'])
